...
METRIC_RUBRIC = {
    ...
    "usefulness": (
        ...
        "补充判定：①个案问题（“我的订单/退款/物流/账号”“这款商品”）只给通用规则、让用户自行查询且没有明确承接"
        "（如“请提供订单号，我来处理”）→ 不超过 3 分；系统具备查询能力却推给用户 → 不超过 2 分；"
        "②用户在表达困难或情绪（“搞不懂”“太复杂”“等了很久”）而回复重复通用流程、未追问具体卡点 → 不超过 2.5 分。"
    ),
    "tone": (
    ...
    "faithfulness": (
        ...
        "判定边界：①“无法核验”不等于编造——无法查证的商品属性等表述最多按 3 分处理，不判 fabrication；"
        "②可由用户自述合理推出的结论（如用户说“用了两周”→ 推断“仍在 30 天质保期内”）不算编造；"
        "③政策类断言与常见政策或数据集内一致口径相符时不扣分，只有与已知口径明显矛盾才扣分；"
        "④“我来帮您查询/处理”类承接：若系统不具备该能力，记入 capability_needs 并在有用性的行动性上扣分，"
        "不判 fabrication。"
    ),
}
